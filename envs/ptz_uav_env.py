...
from env.uav_env import Scenario
...
class PettingZooEnv(BaseEnv):
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self._num_users = self._cfg.n_user #用户数量
    ...
    def reset(self) -> np.ndarray:
        if not self._init_flag:
            ...
            _env = make_env(uav_env_raw_env)
            parallel_env = parallel_wrapper_fn(_env)
            # init env
            self._env = parallel_env(
                N=self._cfg.n_agent,num_user=self._cfg.n_user, num_station=self._cfg.n_landmark, continuous_actions=self._continuous_actions, max_cycles=self._max_cycles
            )
            self._env.reset()
    ...
    def _process_obs(self, obs: 'torch.Tensor') -> np.ndarray:  # noqa
        ...
        # [self_vel, self_pos, landmark_rel_positions, other_agent_rel_positions, user_rel_positions]
        ...
        #              - communication
        ret['agent_state'] = obs
        # global_state: shape=(4 * self._num_agents + 2 * self._num_landmarks + 2 * self._num_users),
        ...
        ret['global_state'] = np.concatenate(
            [
                obs[0, 2:],  # all agents' position + all landmarks' position
        ...
        # agent_alone_state: (self._num_agents, 4 + 2 * self._num_landmarks + 2 * (self._num_agents - 1) + 2 * self._num_users).
...
class uav_env_raw_env(SimpleEnv):
    ...
    def __init__(self, N=3, num_user=2, num_station=2, local_ratio=1, max_cycles=25, continuous_actions=True):
